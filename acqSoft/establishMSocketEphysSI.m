function [sockVis2Ephys,sockEphys2Vis] = establishMSocketEphysSI(d,options)

if nargin < 2
    options = [];
end

ipVis = getOr(options,'partnerIP','128.32.173.25');
inputAddress = getOr(options,'inputAddress',5000);
outputAddress = getOr(options,'outputAddress',6000);
handshake_in = getOr(options,'handshakeIn','port0/line19');
handshake_out = getOr(options,'handshakeOut','port0/line18');
...
disp('SI->Ephys socket established')
%%
go_down(d,shoutat)
...
msclose(srvsock);
disp('Ephys->SI socket established')
